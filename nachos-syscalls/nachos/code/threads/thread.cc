...
//----------------------------------------------------------------------
int NachOSThread::pidnumber=0;   //Initializing pidnumber


int
NachOSThread::PID() //required since pid is a local variable.
{
    return currentThread->pid;
}

int
NachOSThread::PPID()  //Just in case we need PPID in future.
{
    if(currentThread->parent == NULL)
        return -1;
    return currentThread->ppid; 
}
...
NachOSThread::NachOSThread(char* threadName)
{
    ...
    pidnumber++;	//Increment because the first thread is "made by hand"
    pid = pidnumber; 	//assign pidnumber as pid of this thread
    
    if(pid==1){ 
	ppid = 0;    //This is because the main thread will be the parent of pid 1 but it'll not be assigned a pid via this function.
	}
	else{
    ppid = currentThread->PID(); //since this function is called while initializing other thread, the calling thread becomes parent. Since pid is a private variable, we need to use (public)function PID to access it.
	}

    parent=NULL;            //set parent of this thread to NULL..
#ifdef USER_PROGRAM
    space = NULL;
#endif
}
...
int 
NachOSThread::childindex(int pidchild)      //search for the child pid in the childlist. return 1 if found, 0 otherwise.
{
    int i=0;
    for(i=0;i<childcount; i++)
    {
        if(pidchild == childpidlist[i])
            return i;               // child found at index i of childpidlist array
    }
    return -1;              //child not found
}
int 
NachOSThread::childstatusget(int pidchild)     //tells if child has exited(0) or child created but parent not waiting(1) or if parent is waiting(2)
{
    int i = childindex(pidchild);
    if(i==-1)
        return -1;
    else
...
void 
NachOSThread::childstatusset(int pidchild, int status)       //changes exit status of given child.
{
    int i = childindex(pidchild);
    if(i != -1)
...
void 
NachOSThread::AddChildEntry(int pidchild)
{
    childpidlist[childcount] = pidchild;
    childstatus[childcount] = 1;    //parent not waiting and child has been created
    childcount++;
}
void 
ForkFunction(int ignorethis)
{
    //Copied from ..../nachos/code/thread/scheduler.cc
